...
% - Use LaTeX comments or TODO notes for comments and open issues in the text.
...
This paper reports a small controlled benchmark for studying these effects. The goal is not a final leaderboard. Instead, we ask which design choices shape the outcome of LLM-judged persuasive debates. Our current evidence supports three claims. First, stronger cloud judging improves parseability and reduces some local-judge artifacts. Second, rankings change substantially between different-position and same-position comparison, even when both target positions are covered. Third, position, speaking order, and candidate presentation order remain consequential enough that aggregate win rates are unsafe without controls.
...
We use $X=10$ debate topics and $Y=4$ debating models: Apertus-70B, Llama-3.1-8B, Qwen-30B, and Gemma-31B. Each topic specifies two symmetric alternatives, denoted \emph{Position A} and \emph{Position B}. Each debate contains eight alternating messages, corresponding to four turns per model. The assigned starting model produces the first message and the other model produces the final message. Responses are limited to 220 tokens per message.

\subsection{Debate Generation}
For every topic and unordered pair of models, we generate four debates: both possible position assignments (which model defends Position A) crossed with both possible speaking orders (Position A or Position B starts). With $X$ topics and $Y$ models, there are $\binom{Y}{2}=Y(Y-1)/2$ unique model pairs. The total number of debates is therefore
\[
N
= X \cdot \binom{Y}{2} \cdot 2 \cdot 2
= X \cdot \frac{Y(Y-1)}{2} \cdot 4.
\]
For $X=10$ topics and $Y=4$ models, this gives $N=10\cdot6\cdot4=240$ debates.


Failed generations are repeated until one valid debate is obtained for every
condition. Model identities, position assignments, and speaking order are
recorded separately. This distinction is important because model slots are
experimental units, whereas Position~A and Position~B are the two substantive
alternatives being defended.

\subsection{Judgment Generation}

We evaluate the generated debates using three independent judge models:
Gemma-4-31B-IT, Qwen3-30B-A3B-Instruct-2507, and GLM-4.7. Each judge returns a
winner and a self-reported confidence value. We consider two evaluation modes
and pool the three judges unless a result is explicitly judge-specific.

\paragraph{Mode 1: Different-Position Evaluation.}
Each judge receives a single debate transcript and selects which of the two
models argued its assigned position better. This mode resembles an ordinary
debate verdict, but model quality can be confounded with how easy the assigned
position is to defend. Evaluating all 240 debates with three judges yields 720
individual judgments.

The current export additionally stores the winning position and self-reported
confidence for each judge. These fields are retained as diagnostics; the
primary ranking uses only the winner fields.


\paragraph{Mode 2: Same-Position Evaluation.}
To reduce confounding by position difficulty, we construct matched comparisons.
For each topic, unordered model pair
$\{M_i,M_j\}$, tested position $p\in\{A,B\}$, and candidate-speaking condition,
we identify two role-swapped debates
\[
(M_i[p] \text{ vs. } M_j)
\qquad\text{and}\qquad
(M_j[p] \text{ vs. } M_i),
\]
where each candidate defends $p$ in its respective debate and either starts in
both debates or does not start in both. Each judge receives both transcripts and
selects which candidate defended the tested position better. Every underlying
comparison is presented twice with the candidate order reversed.

This mode asks a narrower question than an ordinary debate verdict: which model
made the stronger case for the same proposition? Holding the defended position
and candidate-speaking condition fixed makes the comparison more directly
interpretable as relative argument quality. All judge models evaluate every
presentation.

With $X = 10$ topics and $Y = 4$ models, this gives
\[
  N_{\mathrm{same}}
  = 10 \cdot \binom{4}{2} \cdot 2 \cdot 2 \cdot 2
  = 480
\]
presentations. The factors are 10 topics, six unordered model pairs, two tested
positions, two candidate-speaking conditions, and two candidate orders. Thus,
the design represents 240 underlying comparisons and 480 order-balanced
presentations. Three judges produce $480\cdot3=1440$ individual judgments.
...
For the analyzed run, each model receives the question, both position texts, and
a system instruction assigning exactly one position. The instruction requires
the model to maintain that position, address the opponent's strongest recent
point, and use concrete mechanisms, trade-offs, examples, or failure modes. It
also prohibits references to judges, benchmarks, prompts, or model identities
and discourages unsupported citations and numerical claims. The assigned
speaking order determines the first model; the models then produce
eight alternating messages, four per model. Generation uses a maximum of 220
tokens per message and temperature 0.75 through an OpenAI-compatible cloud API.
The question and symmetric position descriptions serve as the initial
instruction, but are not stored as a moderator turn in the transcript.

\paragraph{Judging Protocol.}
Different-position judges receive one transcript, the two position texts, the
position assignment, and the starting model. They are instructed to evaluate
argument quality, responsiveness, factual discipline, and persuasive clarity,
while ignoring their own policy preference. Same-position judges receive two
transcripts and are instructed to compare only the candidates defending the
specified common position. Candidate order is balanced by presenting every
underlying comparison in both orders. In both modes, ties are allowed and the
judge returns only a model identifier (or \texttt{Tie}) and confidence in
$[0,1]$ as JSON. Judge calls use temperature 0.1 and a 220-token output limit.
The judges do not use external retrieval, so factual discipline is assessed
from the supplied topic and transcripts rather than independent fact checking.

\paragraph{Generation and Parsing Controls.}
API failures during generation and judging are retried, and judge responses are
parsed from structured JSON. Completed judgments are written incrementally so
interrupted runs can resume without replacing valid observations. The data were then
checked for balanced factorial coverage, valid debate references, alternating
speaker order, admissible winner labels, and confidence values in $[0,1]$.
...
We first ask whether a model ranking is a stable property of the models or a
consequence of how persuasion is operationalized. Different-position evaluation
measures success in complete debates, where argument quality, position
difficulty, and interaction dynamics jointly determine the verdict.
Same-position evaluation instead compares two models defending the same
proposition under a matched speaking condition. If the two modes measure the
same underlying capability, their rankings should be similar; large rank changes
indicate that the benchmark conclusion depends on the evaluation construct. We
therefore report rankings separately for every judge and mode and then average
ranks across judges.

\paragraph{Results.}
Table~\ref{tab:current-mode-ranks} gives the primary comparison for
this experiment. Within each judge and mode, models are ranked using the
tie-adjusted score $(\mathrm{wins}+0.5\,\mathrm{ties})/\mathrm{evaluations}$.
The final two columns average each model's rank across the three judges. When
the underlying evaluations are pooled instead, the mean absolute change
between modes is 1.5 rank positions.

\begin{table}[t]
  \centering
  \footnotesize
  \setlength{\tabcolsep}{3pt}
  \caption{\textbf{Exp.~1 -- Evaluation Design:} Model ranks under
  different-position (D) and same-position (S) evaluation, separately for each
  judge and averaged across judges. Rank 1 is best.}
...
The largest change is Llama-3.1-8B, which moves from first under
different-position evaluation to fourth under same-position evaluation.
Gemma-4-31B moves from third to first, Qwen3-30B remains second, and
Apertus-70B moves from fourth to third. Ties occur in 164 of 720
different-position evaluations (22.8\%) but only 4 of 1,440 same-position
evaluations (0.3\%). This difference in tie frequency should accompany the
rank comparison because the ranking uses a fixed half-win treatment for ties.

We next measure whether the verdict is robust to evaluator choice. Agreement is
computed on identical comparison instances, so disagreement cannot be explained
by different debates. High agreement would support treating judges as
interchangeable evaluators; low agreement implies that a reported ranking is
partly judge-specific.

\paragraph{Current Agreement.}
All three judges give the same winner in 21 of 240 different-position rows
(8.8\%) and 254 of 480 same-position rows (52.9\%). In the
different-position mode, 48 rows have three distinct verdicts and therefore no
majority winner; every same-position row has a majority. This is descriptive
agreement among the three current cloud judges, not agreement with human
persuasion judgments.

\paragraph{Debater Confidence.}
Rate the confidence of each model in each debate and score it. Then see how well the judgment (who wins) is correlated with who sounded more confident.
The current data do not contain a validated measure of how confident each
debater sounds, so this relationship is not estimated here.

\paragraph{Current Confidence Diagnostic.}
The existing confidence fields measure judge confidence, not how confident the
debaters sounded, and therefore cannot answer the proposed correlation. Mean
judge confidence in different-position evaluation is 0.838 for Gemma, 0.709
for GLM, and 0.910 for Qwen; in same-position evaluation it is 0.967, 0.730,
and 0.948, respectively. Without a human or otherwise validated correctness
label, these values describe judge certainty but do not establish calibration.

\paragraph{Repeated-Judgment Stability.}
This analysis requires additional judgments but no new debates. We re-evaluate
all $N=240$ transcripts five times with each judge using the same judging prompt.
We report within-judge agreement, the standard deviation of model win rates, and
the frequency with which repeated judgments produce different winners.

\paragraph{Current Stability Diagnostic.}
The present files do not contain five repeated judgments with identical input
order. They do permit a stricter candidate-order check using the same two
underlying debates. The winning model remains unchanged in 190 of 240 paired
reversals for Gemma (79.2\%), 91 for Qwen (37.9\%), and 166 for GLM (69.2\%).
Because the manipulation changes candidate presentation order, this result is
reported as an order effect in Experiment~2 rather than as stochastic
repeatability.

\paragraph{Sensitivity to Style.}
This analysis requires an additional perturbed version of the debate transcripts.
For each transcript, we generate one style-normalized version that preserves the
argument content while making both speakers use the same tone and formatting. We
judge the original and modified transcripts and report the fraction of verdicts
that change.

\paragraph{Current Status.}
No controlled style-normalized or paraphrased transcript variants are present
in the curated files. Natural style differences are confounded with model
identity and argument content, so style sensitivity cannot be estimated from
the current data without the proposed perturbation experiment.

\paragraph{Prompt-Phrasing Sensitivity.}
Judge instructions are part of the measurement instrument: semantically similar
wording may change how a model weighs the same criteria. We therefore re-judge a
balanced 12-debate subset with the canonical instruction and a meaning-preserving
paraphrase, while holding transcripts, candidate labels, output format,
temperature, and token limit fixed. Gemma and Qwen each agree across phrasings on
7 of 12 cases (58.3\%). Across 24 paired comparisons, the 10 changes comprise two
decisive winner reversals and eight transitions between a winner and a tie.
GLM could not complete the paired sample because of repeated API timeouts and is
excluded. Because each prompt receives only one sample per case, this result is
a small cloud diagnostic and cannot separate wording sensitivity from residual
API nondeterminism.

\subsection{Experiment 2: Biases in LLM Persuasion Judging}

\paragraph{Position and Speaking-Order Bias.}
A persuasion benchmark should reward the quality of advocacy, not an arbitrary
role assignment or turn order. The factorial design crosses each model pair with
both position assignments and both speaking orders, allowing us to ask whether
verdict shares change when these nuisance variables change. We measure the share
of decisive judgments won by each position and by the starting speaker. Because
the starter also forfeits the final turn, the latter quantity measures the joint
effect of opening rather than closing; it is not a pure recency estimate.

\paragraph{Current Results.}
Across the 720 different-position judge evaluations, Position~A wins 303,
Position~B wins 253, and 164 are ties. Table~\ref{tab:current-position-order}
reports shares among decisive verdicts. Since the non-starting model also gives
the final statement, the speaking-order statistic contrasts opening with
closing position and does not isolate either mechanism.

\begin{table}[t]
  \centering
  \footnotesize
  \caption{\textbf{Exp.~2.1 -- Position and Speaking-Order Bias:} Position-A
  and starting-speaker shares among decisive different-position verdicts.}
  \label{tab:current-position-order}
  \begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}Xrr@{}}
    \toprule
    Judge & Position A (\%) & Starting speaker (\%) \\
    \midrule
    All judges & 54.5 & 45.5 \\
    Gemma-4-31B & 45.7 & 50.4 \\
    GLM-4.7 & 52.5 & 47.5 \\
    Qwen3-30B & 63.0 & 39.7 \\
    \bottomrule
  \end{tabularx}
...
The position effect is judge-dependent rather than uniform. Candidate
presentation order also matters in the same-position data: Qwen changes the
winning model in 149 of 240 paired candidate-order reversals. This comparison
uses the same two underlying debates in reversed presentation order and is
therefore a direct judge-order diagnostic. By contrast, the position and
speaking-order cells in the raw factorial design contain separately generated
transcripts, so changes across those cells can also reflect generation
variation.
...
\begin{table}[t]
  ...
  \caption{\textbf{Exp.~2.1 -- Candidate-Order Stability:} Stability under
  candidate-order reversal in same-position evaluation. Each pair contains the
  same two underlying debates presented in both candidate orders.}
...
The large variation across judges shows that candidate-order sensitivity is
not a uniform property of the comparison protocol. In particular, Qwen changes
the selected model in most paired reversals, whereas Gemma and GLM retain the
same model more often than not.

\paragraph{Verbosity Bias.}
Verbosity bias matters because a judge may mistake additional text for stronger
reasoning, rewarding output length even when substantive quality is unchanged.
For each decisive judgment, we compare the total word count produced by the
selected model with that of the non-selected model. A larger mean among winners
is the expected observational signature of verbosity preference, but it is not
causal: stronger models or arguments may also be longer. A controlled follow-up
will shorten the longer argument to match its opponent for 100 pre-sampled
debates and re-run the judgments.

\paragraph{Current Results.}
Table~\ref{tab:current-verbosity} pools both evaluation modes and reports one
row per judge. Values are word counts rather than tokenizer-
specific token counts, and variances are sample variances in squared words.

\begin{table}[t]
  \centering
  \footnotesize
  \setlength{\tabcolsep}{4pt}
  \caption{\textbf{Exp.~2.2 -- Verbosity Bias:} Observed argument length for
  winners and losers, pooled across evaluation modes. This is an observational
  diagnostic.}
...
Winners are longer on average for all three judges, but response length is
confounded with model identity, topic, and argument quality. These values do
not establish an independent verbosity bias; that claim requires the matched-
length follow-up described above.

\paragraph{Self-Preference Bias.}
Self-preference would make evaluations systematically more favorable when the
judge and a candidate share a model identity. For each comparison involving
judge-model $X$, we measure how often $X$ selects itself and compare this with
the selection rate of judges that generated neither candidate response on the
same comparison. Using identical cases holds debate difficulty fixed, while
excluding both participants prevents the reference group from containing a
second self-judgment.

\paragraph{Current Results.}
For every row involving judge-model $X$, we compare $X$'s self-selection with
the judgments of models that are not participants in that debate. When two
uninvolved judges are available, their binary selections are averaged so every
debate receives equal weight. Pooling both modes, Gemma selects itself in 217
of 360 rows (60.3\%), compared with a 63.2\% uninvolved-judge baseline on the
same rows. Qwen selects itself in 188 of 360 rows (52.2\%), compared with a
65.4\% uninvolved-judge baseline. Ties count as non-selections. The differences
are $-2.9$ and $-13.2$ percentage points, respectively, so the pooled data do
not show positive self-preference under this diagnostic. These comparisons
remain descriptive because judge-specific evaluation criteria can differ.

\paragraph{Ideological Preference.}
This analysis requires an additional annotation of the debate positions. We select the politically relevant topics and ask three independent annotator models to classify Position A and Position B on a five-point scale from strongly liberal to strongly conservative. The majority annotation is used as the position label. We then compare, for each judge, the probability that the liberal or conservative position wins. Because each model argues both sides and both speaking orders, these effects can be estimated while controlling for debater identity and speaking order.

\paragraph{Current Status.}
The present topics do not have validated ideological annotations. The current
data support judge-specific position preferences, but not a defensible claim
about ideological preference. We therefore do not estimate this effect until an
...
Finally, we ask whether the substantive proposition being defended changes the
chance of winning. This matters because a model leaderboard is misleading if a
model succeeds mainly because it was assigned the more defensible side. For each
topic, we pool verdicts across debating models, role assignments, speaking
orders, and judges. We call the proposition with the larger decisive win share
the \emph{observationally easier} position. Balancing model and order assignments
makes this statistic less dependent on a particular debater, although judge
preferences and generation variation still prevent a causal or human-validated
interpretation.

\paragraph{Current Results.}
Table~\ref{tab:current-position-difficulty} shows the four largest observed
departures from an even split. Counts pool the three judges and are descriptive
rather than human-validated measures of intrinsic position difficulty.

\begin{table}[t]
  \centering
  \footnotesize
  \caption{\textbf{Exp.~3 -- Position Difficulty:} Topics with the largest
  observed asymmetries. Easy share is the easier proposition's share among
  decisive verdicts; the final column gives easy-position wins, hard-position
  wins, and ties.}
...
decentralized renewable microgrids win 40 decisive evaluations, whereas
centralized nuclear fission wins 18. The
current ten-topic set does not contain a deliberately obvious moral-control
item. Such an extreme case should be added only after its exact wording and
selection criteria are pre-specified.



\subsection{(Optional) Experiment 4: Comparison with Human Debate Judgments}
...
Recent work connects persuasion directly to evaluator reliability.
\citet{zahraei2026prior} show that LLM judges can conflate prior agreement with
argumentative quality, including in debate evaluation. \citet{carro2025beliefs}
compare sequential and simultaneous subjective debates and report a
second-speaker effect in the sequential protocol. Multi-turn frameworks also
study both persuasive effectiveness and susceptibility
\citep{bozdag2025persuade}. Separately, \citet{hwang2025trick} demonstrate that
rhetorical insertions can alter LLM-judge scores and pairwise verdicts, while
\citet{bandaru2025political} use structured multi-agent debate to study how model
and persona choices shape political attitudes. Together, this literature
motivates treating the evaluation prompt, judge identity, and interaction
protocol as parts of the measurement design.
...
The current study shows that rankings from LLM-judged debates depend on both the
comparison protocol and judge identity. Future experiments will extend the panel
with one substantially stronger and one substantially weaker model and will use
the same model set for debate generation and judgment. Additional priorities are
forced-choice judging without ties, prompt-phrasing sensitivity, deliberately
easy control debates, and human validation.
