...
We use $X=10$ topics and $Y=4$ debating models: Apertus-70B, Llama-3.1-8B, Qwen-30B, and Gemma-31B. Each topic defines symmetric \emph{Position A} and \emph{Position B} alternatives. A debate contains eight alternating messages, four per model, limited to 220 tokens each.

\subsection{Debate Generation}
For every topic and unordered model pair, we cross both position assignments with both speaking orders. The design therefore contains $N=X\binom{Y}{2}\cdot4=10\cdot6\cdot4=240$ debates. Failed generations are repeated until every condition has one valid transcript. Model identity, position, and order are recorded separately so model slots are not confused with the substantive alternatives.

\subsection{Judgment Generation}

Three judges evaluate the debates: Gemma-4-31B-IT, Qwen3-30B-A3B-Instruct-2507, and GLM-4.7. Each returns a winner and self-reported confidence. We pool judges unless reporting judge-specific results.
...
To reduce position confounding, we match role-swapped debates in which each candidate defends the same position and either starts in both transcripts or does not start in both. Judges select which candidate made the stronger case for that proposition. Each underlying comparison is shown in both candidate orders. The resulting $10\binom{4}{2}\cdot2\cdot2\cdot2=480$ presentations represent 240 matched comparisons and yield $480\cdot3=1{,}440$ judgments.
...
Each model receives the question, both position texts, and one assigned position. It must maintain that position, answer the opponent's strongest recent point, and use concrete mechanisms, trade-offs, examples, or failure modes. References to judges, prompts, benchmarks, and model identities are prohibited. Generation uses temperature 0.75 and a 220-token limit per message through an OpenAI-compatible cloud API.

\paragraph{Judging Protocol.}
Judges score argument quality, responsiveness, factual discipline, and persuasive clarity while ignoring personal policy preference. Same-position judges compare only the two candidates defending the specified proposition. Ties are allowed; output is JSON containing a model identifier (or \texttt{Tie}) and confidence in $[0,1]$. Calls use temperature 0.1 and a 220-token limit. Without retrieval, factual discipline is assessed only from the supplied topic and transcripts.

\paragraph{Generation and Parsing Controls.}
API failures are retried and valid JSON judgments are stored incrementally. We verify factorial coverage, debate references, alternating turns, winner labels, and confidence values.
...
The two protocols do not produce a common ordering. Llama-3.1-8B has the best mean rank in different-position evaluation, although the Qwen judge ranks it last, but all three judges rank it last in same-position evaluation. Gemma-4-31B becomes the strongest model on average in the matched task, while Qwen3-30B remains comparatively stable in second place. Because same-position evaluation holds the defended proposition and speaking condition fixed, this reversal suggests that success in a complete debate does not transfer directly to the narrower ability to make the stronger case for a shared proposition. A single pooled leaderboard would conceal this distinction.

The protocols also differ in decisiveness. Ties account for 164 of 720 different-position evaluations (22.8\%) but only 4 of 1,440 same-position evaluations (0.3\%). The higher tie rate in complete debates is consistent with judges finding opposing-position performances harder to compare directly. Since ties receive a fixed half-win in the ranking, this difference is part of the measurement design rather than a minor reporting detail.
...
A persuasion benchmark should reward the quality of advocacy, not an arbitrary role assignment or turn order. The factorial design crosses each model pair with both position assignments and both speaking orders, allowing us to ask whether verdict shares change when these nuisance variables change. We measure the share of decisive judgments won by each position and by the starting speaker. Because the starter also forfeits the final turn, the latter quantity measures the joint effect of opening rather than closing; it is not a pure recency estimate.

\paragraph{Results.}
Across the 720 different-position judge evaluations, Position~A wins 303, Position~B wins 253, and 164 are ties. Table~\ref{tab:current-position-order} reports shares among decisive verdicts. Since the non-starting model also gives the final statement, the speaking-order statistic contrasts opening with closing position and does not isolate either mechanism.

\begin{table}[t]
  \centering
  \footnotesize
  \caption{\textbf{Exp.~2.1 -- Position and Speaking-Order Bias:} Position-A and starting-speaker shares among decisive different-position verdicts.}
...
The position effect is judge-dependent rather than uniform. Candidate presentation order also matters in the same-position data: Qwen changes the winning model in 149 of 240 paired candidate-order reversals. This comparison uses the same two underlying debates in reversed presentation order and is therefore a direct judge-order diagnostic. By contrast, the position and speaking-order cells in the raw factorial design contain separately generated transcripts, so changes across those cells can also reflect generation variation.
...
The large variation across judges shows that candidate-order sensitivity is not a uniform property of the comparison protocol. In particular, Qwen changes the selected model in most paired reversals, whereas Gemma and GLM retain the same model more often than not.
...
\paragraph{Results.}
Table~\ref{tab:current-verbosity} pools both evaluation modes and reports one row per judge. Values are word counts rather than tokenizer- specific token counts, and variances are sample variances in squared words.

\begin{table}[t]
  \centering
  \footnotesize
  \setlength{\tabcolsep}{4pt}
  \caption{\textbf{Exp.~2.2 -- Verbosity Bias:} Observed argument length for winners and losers, pooled across evaluation modes. This is an observational diagnostic.}
...
Winners are longer on average for all three judges, but response length is confounded with model identity, topic, and argument quality. These values do not establish an independent verbosity bias; that claim requires the matched- length follow-up described above.
...
\paragraph{Results.}
For every row involving judge-model $X$, we compare $X$'s self-selection with the judgments of models that are not participants in that debate. When two uninvolved judges are available, their binary selections are averaged so every debate receives equal weight. Pooling both modes, Gemma selects itself in 217 of 360 rows (60.3\%), compared with a 63.2\% uninvolved-judge baseline on the same rows. Qwen selects itself in 188 of 360 rows (52.2\%), compared with a 65.4\% uninvolved-judge baseline. Ties count as non-selections. The differences are $-2.9$ and $-13.2$ percentage points, respectively, so the pooled data do not show positive self-preference under this diagnostic. These comparisons remain descriptive because judge-specific evaluation criteria can differ.
...
\paragraph{Results.}
Table~\ref{tab:current-position-difficulty} shows the four largest observed departures from an even split. Counts pool the three judges and are descriptive rather than human-validated measures of intrinsic position difficulty.
...
Energy infrastructure is the strongest current numerical asymmetry: decentralized renewable microgrids win 40 decisive evaluations, whereas centralized nuclear fission wins 18. The current ten-topic set does not contain a deliberately obvious moral-control item. Such an extreme case should be added only after its exact wording and selection criteria are pre-specified.
