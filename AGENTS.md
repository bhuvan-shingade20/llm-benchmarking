...
- Batch runs default to two model-role assignments: configured Position A/B models, then reversed models. The legacy flag `--no-side-swap` disables that reversed run.
- With `--models`, `run_benchmark.py` runs all ordered model-role permutations, so use `--dry-run` first to avoid unexpected API cost.
...
## Coupled Changes
- If result CSV/JSON columns change in `run_benchmark.py`, update `analyze_results.py` and the README analysis docs in the same change.
